...
const columns: Array<v2ColumnInterface[]> = [[
    ...
    { label: "Curr.Reg", ref: "current_regimen", defaultValue: "N/A" },
    { label: "ARVs", ref: "medication", toValue: (val) => val.map((d: any) => `${d.medication} (${d.quantity})`).join(', ')},
...
const generate = () => wrapGeneration(async () => {
    ...
    reportData.value = []
    const res: any = await report.getRegimenReport();
    reportData.value = Object.values(res).map((d: any) => ({ 
        ...d, last_dispensation_date: d.medication.reduce((a: any, c: any) => c.start_date, '')
    }))
})
