export default {
    'Resistance classification': 9984,
    'Mono drug resistant': 9982,
    'Multi drug resistant': 9983,
    'Patient tracking state': 6313,
    'Standard intensive phase regimen': 9973,
    'Standard continuation  phase regimen': 9960,
    'Shorter intensive phase regimen': 9959,
    'Shorter continuation phase regimen': 9958,
    'Individualised regimen': 9969,
    'Short intensive extended regimen': 9968,
    'Cough lasting >1 week': 9694,
    'Cough lasting >2 weeks': 2573,
    'Cough (any duration)': 8261,
    'Weight loss': 832,
    'Fever lasting >1 week': 8279,
    'Fever lasting >2 weeks': 9689,
    'Fever (any duration)': 9690,
    'Profuse night sweats lasting >1 week': 9691,
    'Profuse night sweats lasting >2 weeks': 9692,
    'Profuse night sweats (any duration)': 9693,
    APPOINTMENT_DATE: 5096,
    ESTIMATED_DATE: 7437,
    HIV_BEFORE_TB: 9986,
    HIV_AFTER_TB: 9987,
    ARV_BEFORE_TB: 9952, 
    ARV_WHILE_TB: 9988,
    ARV_AFTER_DISCHARGE: 9989,
    TIME_OF_HIV_TEST: 9985,
    ARV_STATUS: 9990,
    PATIENT_IS_PREGNANT: 1755,
    YES_ANSWER: 1065,
    NO_ANSWER: 1066,
    SYMPTOMATIC_CONTACT_OF_DR_PATIENT_WHO_DIED: 9949,
    TB_SCREENING_CRITERIA: 9698,
    'OPD / MCH Clinic / HIV Negative': 9695,
    'Mining Communities': 9696,
    'HIV Clinic / PLHIV / Prisoners': 9697,
    MDR_TB_CONTACT: 1536,
    HTS_CLINIC: 7796,
    PRIVATE_PRACTITIONER: 8349,
    SPUTUM_COLLECTION_POINT: 8350,
    PMTCT: 1939,
    WALK_IN: 1791,
    SOURCE_OF_REFERRAL: 8021,
    NEGATIVE: 664,
    MCH: 2232,
    OPD: 7799,
    MINING_COMMUNITIES: 9696,
    ART_CLINIC: 7054,
    POSITIVE: 703,
    PRISONER: 2007,
    WARD: 7829,
    HIV_TEST_DATE: 1837,
    PREVIOUSLY_TESTED: 9656,
    PREVIOUS_TEST_DATE: 9657,
    ON_ART: 1577,
    HIV_STATUS: 3753,
    HIV_TEST_DONE_TODAY: 9568,
    NOT_DONE: 1118,
    REASON_HIV_TEST_NOT_DONE: 9569,
    LIVES_NEAR: 9166,
    TB_SYMPTOMS: 1560,
    PRESCRIBE_DRUGS: 7874,
    MEDICAL_ORDERS: 1282,
    AMOUNT_DISPENSED: 2834,
    X_RAY: 6687,
    CLINICAL: 3592,
    TB_STATUS: 7459,
    PROCEDURE_TYPE: 9587,
    TB_ADHERENCE: 6987,
    DRUGS_BROUGHT: 2540,
    TB_DRUGS_START_DATE: 1113,
    HEALTH_WORKER: 1538,
    PREV_TEST_DATE: 9657,
    COUGH_DURATION: 5959,
    OCCUPATION: 1304,
    TRANSFERRED_OUT_EXTERN: 9230,
    TRANSFERRED_IN: 8365,
    FACILITY_NAME: 8341,
    TRANSFERRED_OUT_STATE: 95,
    REGIMEN_SUPPLY_DAYS: 3640,
    REGIMEN_TYPE: 9961,
    PRIMARY_REGIMEN: 1155,
    OTHER_REFERRAL: 6408,
    RESULT_DATE: 3045,
    LAB_ORDER: 3388,
    PULMONARY_TB: 1549,
    EXTRA_PULMONARY_TB: 1547,
    TB_TYPE: 2596,
    DIAGNOSIS_DATE: 2182,
    TB_NUMBER: 7930,
    MDR_NUMBER:9975,
    APPOINTMENT_REASON: 6784,
    ULTRASOUND: 6686,
    MENINGITIS_TB: 6628,
    PREVIOUSLY_ON_FIRST_LINE_DRUGS: 3523,
    TUBERCULOSIS_CLASS: 7932,
    'Lab test result': 7363,
    'Estimated turnaround time for result': 9981,
    'Reason for exiting care': 1811,
    'Rifampicin resistance confirmed': 9980,
    'Confirmatory Rifampicin Resitance test': 9979,
    'Pleural Effusion': 1136,
    'Pericardial Effusion': 6875,
    'Abdominal Tuberculosis': 9786,
    'Tuberculosis Ascites': 581,
    'Tuberculosis Spine': 7509,
    'Infiltration': 7684,
    'Tuberculosis Miliary': 7511,
    'Opacification': 9787,
    'Consolidation': 9788,
    'Cavitation': 9789,
    'Hilar Lymphadenopathy': 9790,
    'Tuberculosis Meningitis': 6628,
    'Lymphadenitis': 3736,
    'Gastrointestinal': 7661,
    'Osteoarticular': 9873,
    LAM: 9791,
    'All Other': 6408,
    CLINICALLY_DIAGNOSED: 9792,
    BACTERIOLOGICALLY_DIAGNOSED: 9793,
    SAMPLE: 6680,
    REASON_FOR_TEST: 2429,
    TEST_REQUESTED: 6660,
    HAVE_YOU_HAD_TB: 8266,
    TREATMENT_FAILURE: 843,
    RELAPSE: 9814,
    NEW_TB_CASE: 2406,
    PATIENT_CATEGORY: 7935,
    TREATMENT_DURATION: 3515,
    NEW_PATIENT: 2406,
    REFERRAL: 9675,
    PATIENT_TYPE: 3289,
    UNKNOWN: 1067,
    DEFAULTER: 6828,
    ANTIRETROVIRAL_TREATMENT_NEED: 1445,
    CULTURE_AND_DST: 2508,
    TB_CULTURE_RESULT: 3046,
    MICROSCOPY: 3050,
    XPERT_MTB_RIF: 9815, 
    XPERT_MTB_RIF_ULTRA: 9816,
    LPA: 9817,
    TEST_TYPE: 9737,
    TB: 7691,
    'Spit': 1004,
    'Blood': 8612,
    'Sputum': 1004,
    'CSF': 1594,
    'Pleural Fluid': 6690,
    'Ascitic Fluid': 6691,
    'Pericardial Fluid': 6692,
    'Peritoneal Fluid': 2483,
    'Pus': 9820,
    'Urine': 1592,
    DIAGNOSIS: 3065,
    FOLLOW_UP: 2484,
    LAB: 9739,
    AREA_WITH_PREVALENCE_OF_DR_TB: 9948,
  // Tuberculosis smear microscopy method
    SAMPLE_ONE_GENEXPERT_RESULT: 9834,
    SAMPLE_TWO_GENEXPERT_RESULT: 9835,
    SAMPLE_ONE_RIF_RESISTANCE_PATTERN: 9836,
    SAMPLE_TWO_RIF_RESISTANCE_PATTERN: 9837,
    SAMPLE_ONE_GENEXPERT_RESULT_DATE: 9838,
    SAMPLE_TWO_GENEXPERT_RESULT_DATE: 9839,
    MTB_DETECTED: 9821,
    MTB_NOT_DETECTED: 9822,
    MTB_TRACE: 9826,
    MTB_INVALID: 9828,
    MTB_NO_RESULT: 9829,
    MTB_ERROR: 9827,
    RIF_RESISTANT_DETECTED: 9823,
    RIF_RESISTANT_NOT_DETECTED: 9824,
    RIF_RESISTANT_INDETERMINATE: 9825,
    ISONIAZID_RESISTANT_DETECTED: 9951,
    SAMPLE_ONE_MICROSCOPY_PHYSICAL_APPEARANCE: 9840,
    SAMPLE_TWO_MICROSCOPY_PHYSICAL_APPEARANCE: 9841,
    SAMPLE_ONE_MICROSCOPY_PROCEDURE: 9842,
    SAMPLE_TWO_MICROSCOPY_PROCEDURE: 9843,
    SAMPLE_ONE_MICROSCOPY_RESULT: 9844,
    SAMPLE_TWO_MICROSCOPY_RESULT: 9845,
    SAMPLE_ONE_MICROSCOPY_GRADING: 9846,
    SAMPLE_TWO_MICROSCOPY_GRADING: 9847,
    SAMPLE_ONE_MICROSCOPY_RESULT_DATE: 9850,
    SAMPLE_TWO_MICROSCOPY_RESULT_DATE: 9851,
    SAMPLE_ONE_NUMBER_OF_BACILI_SEEN: 9848,
    SAMPLE_TWO_NUMBER_OF_BACILI_SEEN: 9849,
    TB_DRUG_RESISTANT: 1597,
    AFB_POSITIVE: 9832,
    AFB_NEGATIVE: 9833,
    ZIEHL_NEELSEN: 9831,
    FLUORESCENCE: 9830,
    SALIVA: 2487,
    MUCO_PURULENT: 2486,
    BLOOD_STAINED: 1077,
    OTHER: 6408,
    XPERTRIF: 9815,
    SPECIMEN_TYPE: 9738,
    ART_START_DATE: 2516,
    CPT: 916,
    ON_CPT: 7024,
    KNOWN_RESISTANCE: 2183,
    CPT_START_DATE: 9856,
    MOBILE_CLINIC: 9857,
    RETURN_AFTER_LOST_TO_FOLLOW_UP: 9859,
    GUARDIAN: 3565,
    HEALTH_CARE_WORKER: 1538,
    VOLUNTEER: 8351,
    TYPE_OF_SUPPORT: 3105,
    HSA: 8872,
    MALAWI_TB_SIDE_EFFECTS: 9866,
    ABNOMINAL_PAIN: 9872,
    ATHRALGIA: 9867,
    COLOUR_BODY_FLUIDS: 9862,
    DEPRESSION: 207,
    FATIGUE: 5949,
    GASTROINTESTINAL: 7661,
    HEADACHE: 620,
    HEARING_DIFFICULTIES: 861,
    HEMATOLOGICAL: 9871,
    HEPATOXICITY: 9864,
    HYPOTHYROIDISM: 6548,
    SKIN_ITCHING: 7868,
    JAUNDICE: 215, //  other
    LACTIC_ACIDOSIS: 1458,
    NUMBNESS_IN_HANDS_FEET_LEGS: 6390,
    OPTIC_NEURITIS: 9868,
    PANCREATITIS: 7689,
    PERIFERAL_NEUROPATHY: 9865,
    PSYCHOSIS: 219,
    QT_PROLONGATION: 9869,
    RENAL_FAILURE: 3681,
    SKIN_RASH: 512,
    TOXICITY: 1756,
    CONTRAINDICATIONS: 9661,
    DRUG_INDUCED: 7567,
    PATIENT_WEIGHT: 5089,
    EX_MINERS: 9943,
    CURRENT_MINERS: 9942,
    NONE: 1107,
    COMMUNITY_SPUTUM_COLLECTION_POINT: 9944,
    HOUSE_TO_HOUSE_TB_SCREENING: 9945,
    HH_TB_SCREENING_SITES: 9946,
    PREVENTIVE_TREATMENT_STATE: 173,
    NEW_MDR_PATIENT: 1527,
    MDR_TB_REGISTRATION_GROUP: 1531,
    MDR_TREATMENT_START_DATE: 1453,
    TREATED_WITH_FIRST_LINE_DRUGS: 3523,
    TREATED_WITH_SECOND_LINE_DRUGS: 3522,
    TRANSFERRED_IN_MDR_PATIENT: 1529,
    RR_CONFIRMED : 9980
  } as Record<string, number>