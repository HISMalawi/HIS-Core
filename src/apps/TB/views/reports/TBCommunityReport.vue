<template>
  <ion-page>
    <v2Datatable
      title="Community Interventions Report"
      :subtitle="subtitle"
      :columnData="data"
      :columns="columns"
      :on-finish="onFinish"
...
export default defineComponent({
  ...
  setup() {
    ...
    const toDrillColumn = (label: string, ref: string) => {
      ...
      if (
        props.data.indicator == 'number_of_newly_established_sputum_sample_collection_points' ||
        props.data.indicator == 'functional_sputum_sample_collection_points'
      ) {
        return;
      }
      (
    ...
    const reportIndicators = ref<Option[]>([
      {
        label: "Number of Presumptive Cases Referred from CSCP",
        value: "number_of_presumptive_tb_cases_referred_from_cscp",
      },
      {
        label: "Number of Presumptive TB Cases Referred from HH TB Screening Sites",
        value: "number_of_presumptive_tb_cases_referred_from_hh_tb_screening_sites",
      },
      {
        label: "Number of TB Cases Diagnosed among Sputum Collection Point Referrals",
        value: "number_of_tb_cases_diagnosed_among_sputum_collection_point_referrals",
      },
      {
        label:
          "Number of TB Cases Diagnosed among Referrals from House to House TB Screening",
        value: "number_of_tb_cases_diagnosed_among_sputum_collection_point_referrals",
      },
      {
        label:
          "Number of TB Cases Diagnosed among Referrals from Mobile Diagnostic Units",
        value:
          "number_of_tb_cases_diagnosed_among_referrals_from_house_to_house_tb_screening",
      },
      {
        label: "Number of newly established sputum sample collection points",
        value: "number_of_newly_established_sputum_sample_collection_points",
      },
      {
        label: "Functional Sputum sample collection points",
        value: "functional_sputum_sample_collection_points",
      },
    ]);
    ...
    const generate = async () => {
      ...
      try {
        ...
        const results = await report_service.getCommunityReport(
          yearValue.value,
          quarterValue.value,
