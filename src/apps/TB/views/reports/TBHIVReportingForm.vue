...
export default defineComponent({
  ...
  setup() {
    ...
    const reportIndicators = ref<Option[]>([
      {
        label: "Total with HIV Result Documented",
        value: "total_with_hiv_result_documented",
      },
      {
        label: "New and Relapse TB Cases Notified",
        value: "new_and_relapse_tb_cases_notified",
      },
      { label: "Total Tested HIV Positive", value: "total_tested_hiv_positive" },
      { label: "Started CPT", value: "started_cpt" },
      {
        label: "Started ART before TB Treatment",
        value: "started_art_before_tb_treatment",
      },
      {
        label: "Started ART while on Treatment",
        value: "started_art_while_on_treatment",
      },
    ]);
    const columns: Array<v2ColumnInterface[]> = [
      [
        ...
        toDrillColumn("Male", "male"),
        toDrillColumn("Female", "female"),
    ...
    const generate = async () => {
      ...
      try {
        ...
        const results = await report_service.getTBHIVReportingForm(
          yearValue.value,
          quarterValue.value,
    ...
    const deSelectAll = () => {
      return reportIndicators.value.map((l) => {
        l.isChecked = false;
        return l;
      });
    };
